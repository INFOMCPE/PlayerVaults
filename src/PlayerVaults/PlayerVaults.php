        ...
        if(isset($args[0]) && $args[0] !== "help" && $args[0] !== ""){
            if(is_numeric($args[0])){
                if(strpos($args[0], ".") !== false){
                    $sender->sendMessage(TF::RED."Пожалуйста укажите номер");
                }elseif($args[0] < 1 || $args[0] > $this->getMaxVaults()){
                    $sender->sendMessage(TF::YELLOW."Использывание: ".TF::WHITE."/pv <1-".$this->getMaxVaults().">");
                }else{
                    if($sender->y + Provider::INVENTORY_HEIGHT > Level::Y_MAX){
                        $sender->sendMessage(TF::RED."Невозможно открыть хранилище на этой высоте. Пожалуйста, опуститесь ниже Y=".Level::Y_MAX - Provider::INVENTORY_HEIGHT);
                    }else{
                        if($sender->hasPermission("playervaults.vault.".$args[0])){
                            $sender->sendMessage(TF::YELLOW."Открытие хранилища ".TF::AQUA."#".$args[0]."...");
                            $this->getData()->sendContents($sender, $args[0]);
                        }else{
                            $sender->sendMessage(TF::RED."Увас нет прав доступа к хранилишю #".$args[0]);
                        }
                    }
                }
            }else{
                if($sender->isOp()){
                    switch(strtolower($args[0])){
                        case "of":
                            if(!isset($args[1])){
                                $sender->sendMessage(TF::RED."Использывание: /$cmd of <player> <number=1>");
                            }else{
                                ...
                                if(!is_numeric($args[2])){
                                    $sender->sendMessage(TF::RED."Использывание: /$cmd of <player> <1-".$this->getMaxVaults().">");
                                    break;
                                }
                                $this->getData()->sendContents($args[1], $args[2] ?? 1, $sender->getName());
                                $sender->sendMessage(TF::YELLOW."Открытие хранилища ".TF::AQUA."#".($args[2] ?? 1)." of ".($player ?? $args[1])."...");
                            }
                            break;
                        case "empty":
                            if(!isset($args[1])){
                                $sender->sendMessage(TF::RED."Использывание: /$cmd empty <player> <number|all>");
                            }else{
                                ...
                                if(!isset($args[2]) || ($args[2] != "all" && !is_numeric($args[2]))){
                                    ...
                                    if((is_numeric($args[2]) && ($args[2] >= 1 || $args[2] <= $this->getMaxVaults())) || $args[2] == "all"){
                                        ...
                                        if($args[2] == "all"){
                                            $sender->sendMessage(TF::YELLOW."Удалены все хранилища ".($player ?? $args[1]).".");
                                        }else{
                                            $sender->sendMessage(TF::YELLOW."Удалены ".($player ?? $args[1])."'s хранилище #".$args[2].".");
                                        }
                                    }else{
                                        $sender->sendMessage(TF::RED."Использывание: /$cmd empty ".$args[1]." <1-".$this->getMaxVaults().">");
                                    }
                                }
                            }
                            break;
                ...
                switch(strtolower($args[0])){
                    case "about":
                        $sender->sendMessage(implode(TF::RESET.PHP_EOL, [
                            ...
                            TF::YELLOW."Перевод на русский совершил: vk.com/olekseyua (INFOMCPE.RU)"
                        ]));
                        break;
                    case "admin":
                        $sender->sendMessage(implode(TF::RESET.PHP_EOL, [
